...
> **Phase P0 closed 2026-07-19.** Gate 0 met, `gate-0-scaffold` tagged, PR #1 and PR #2 merged.
>
> **This file is frozen history and is not maintained.** The live handoff is `HANDOFF.md`,
> which now describes P1. Nothing here has been edited to look better in retrospect: the
> red-gate output, the blocking question that stopped the build, and the two CI failures are
> preserved verbatim, because a ledger that tidies its own failures is a screenshot.
...
**Tag:** `gate-0-scaffold`
**Verified on a fresh clone from GitHub**, detached at the tag, with no local git config —
which is the only check that matters for a tag whose purpose is *clone it and run the gate*:
