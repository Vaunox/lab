...
| **`HANDOFF.md`'s protection block is a live gate input, and `HANDOFF.md` resets** | **every phase close, forever** — not a decision, a standing hazard | The frozen manifest cites `certifying_test: HANDOFF.md::piped_gh_api_output` for `P0.BOOT.PROTECTION`, resolved by `check_manifest._check_evidence_block` against the `<!-- piped_gh_api_output -->` marker and the first fenced JSON block after it. **`HANDOFF.md` is archived and reset at every phase close, so that block must be carried across verbatim every time.** It is not P0 history; it is a live gate input, and §3.3 explains why it can never be machine-fetched instead (verifying branch protection needs an admin-scoped token, and putting one in CI to check that admins are restricted is a circle not worth closing). **If the `manifest` stage goes red on `P0.BOOT.PROTECTION` after a phase reset, the block was dropped — restore it.** Do **not** relax `check_manifest`, and do **not** edit or delete the manifest row: it lives in a frozen deep dive, editing it in a code PR fails `check_spec_isolation` (§11.1), and it is precisely the *"cheapest way to pass a manifest gate is to edit the manifest"* move §8 exists to refuse. An inline warning sits above the marker in `HANDOFF.md`; **this row is the copy that outlives the reset which deletes that warning.** |
